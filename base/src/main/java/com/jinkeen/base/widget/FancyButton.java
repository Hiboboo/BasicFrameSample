package com.jinkeen.base.widget;
...
import com.jinkeen.base.util.ResourceKt;
import com.jinkeen.base.util.UtilsKt;
...
public class FancyButton extends LinearLayout {
    ...
    private int mGradientDefaultBackgroundStartColor;
    private int mGradientDefaultBackgroundEndColor;
    private int mGradientFocusBackgroundStartColor;
    private int mGradientFocusBackgroundEndColor;
    ...
    private final int STYLE_BOLD = 2;
    private final int STYLE_ITALIC = 3;

    private int mDefaultTextColor = Color.WHITE;
    private int mDefaultIconColor = Color.WHITE;
    private int mTextPosition = 1;
    private int mDefaultTextSize = (int) UtilsKt.spToPx(15, getContext());
    private int mDefaultTextGravity = 0x11; // Gravity.CENTER
    private int mTextStyle = STYLE_NORMAL;
    private String mText = null;

    // # Icon Attributes
    private Drawable mIconResource = null;
    private int mFontIconSize = (int) UtilsKt.spToPx(15, getContext());
    private String mFontIcon = null;
    private int mIconPosition = 1;
    private boolean isZoomIcon = false;
    private int zoomIconWidth = 0;
    private int zoomIconHeight = 0;
    ...
    public FancyButton(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private TextView setupTextView() {
        ...
        textView.setTextSize(UtilsKt.toSp(mDefaultTextSize, getContext()));
    ...
    private TextView setupFontIconView() {
        ...
        if (mFontIcon != null) {
            ...
            if (!isInEditMode()) {
                fontIconView.setTextSize(UtilsKt.toSp(mFontIconSize, getContext()));
    ...
    private ImageView setupIconView() {
        if (mIconResource != null) {
            ...
            if (isZoomIcon) {
                iconView.setImageDrawable(ResourceKt.zoom(mIconResource, zoomIconWidth, zoomIconHeight));
            } else
            ...
            if (mTextView != null) {
                if (mIconPosition == POSITION_TOP || mIconPosition == POSITION_BOTTOM)
                    iconViewParams.gravity = Gravity.CENTER_HORIZONTAL;
                else
     ...
     *
     * @param a : Attributes array
     */
    private void initAttributesArray(TypedArray a) {
        mDefaultBackgroundColor = a.getColor(R.styleable.FancyButton_fb_defaultColor, mDefaultBackgroundColor);
        mFocusBackgroundColor = a.getColor(R.styleable.FancyButton_fb_focusColor, mFocusBackgroundColor);
        mDisabledBackgroundColor = a.getColor(R.styleable.FancyButton_fb_disabledColor, mDisabledBackgroundColor);

        isGradient = a.getBoolean(R.styleable.FancyButton_fb_isGradient, false);
        mGradientDefaultBackgroundStartColor = a.getColor(R.styleable.FancyButton_fb_default_start_color, mDefaultBackgroundColor);
        mGradientDefaultBackgroundEndColor = a.getColor(R.styleable.FancyButton_fb_default_end_color, mDefaultBackgroundColor);
        mGradientFocusBackgroundStartColor = a.getColor(R.styleable.FancyButton_fb_focus_start_color, mFocusBackgroundColor);
        mGradientFocusBackgroundEndColor = a.getColor(R.styleable.FancyButton_fb_focus_end_color, mFocusBackgroundColor);
        ...
        mEnabled = a.getBoolean(R.styleable.FancyButton_android_enabled, isEnabled());
        super.setEnabled(mEnabled);

        mDisabledTextColor = a.getColor(R.styleable.FancyButton_fb_disabledTextColor, mDisabledTextColor);
        mDisabledBorderColor = a.getColor(R.styleable.FancyButton_fb_disabledBorderColor, mDisabledBorderColor);
        mDefaultTextColor = a.getColor(R.styleable.FancyButton_fb_textColor, mDefaultTextColor);
        // if default color is set then the icon's color is the same (the default for icon's color)
        mDefaultIconColor = a.getColor(R.styleable.FancyButton_fb_iconColor, mDefaultTextColor);

        mDefaultTextSize = (int) a.getDimension(R.styleable.FancyButton_fb_textSize, mDefaultTextSize);
        mDefaultTextSize = (int) a.getDimension(R.styleable.FancyButton_android_textSize, mDefaultTextSize);

        mDefaultTextGravity = a.getInt(R.styleable.FancyButton_fb_textGravity, mDefaultTextGravity);

        mBorderColor = a.getColor(R.styleable.FancyButton_fb_borderColor, mBorderColor);
        mBorderWidth = (int) a.getDimension(R.styleable.FancyButton_fb_borderWidth, mBorderWidth);

        mRadius = (int) a.getDimension(R.styleable.FancyButton_fb_radius, mRadius);

        mRadiusTopLeft = (int) a.getDimension(R.styleable.FancyButton_fb_radiusTopLeft, mRadius);
        mRadiusTopRight = (int) a.getDimension(R.styleable.FancyButton_fb_radiusTopRight, mRadius);
        mRadiusBottomLeft = (int) a.getDimension(R.styleable.FancyButton_fb_radiusBottomLeft, mRadius);
        mRadiusBottomRight = (int) a.getDimension(R.styleable.FancyButton_fb_radiusBottomRight, mRadius);

        mFontIconSize = (int) a.getDimension(R.styleable.FancyButton_fb_fontIconSize, mFontIconSize);

        mIconPaddingLeft = (int) a.getDimension(R.styleable.FancyButton_fb_iconPaddingLeft, mIconPaddingLeft);
        mIconPaddingRight = (int) a.getDimension(R.styleable.FancyButton_fb_iconPaddingRight, mIconPaddingRight);
        mIconPaddingTop = (int) a.getDimension(R.styleable.FancyButton_fb_iconPaddingTop, mIconPaddingTop);
        mIconPaddingBottom = (int) a.getDimension(R.styleable.FancyButton_fb_iconPaddingBottom, mIconPaddingBottom);

        mTextAllCaps = a.getBoolean(R.styleable.FancyButton_fb_textAllCaps, false);
        mTextAllCaps = a.getBoolean(R.styleable.FancyButton_android_textAllCaps, false);

        mGhost = a.getBoolean(R.styleable.FancyButton_fb_ghost, mGhost);
        mUseSystemFont = a.getBoolean(R.styleable.FancyButton_fb_useSystemFont, mUseSystemFont);

        String text = a.getString(R.styleable.FancyButton_fb_text);

        if (text == null) { //no fb_text attribute
            text = a.getString(R.styleable.FancyButton_android_text);
        }

        mTextStyle = a.getInt(R.styleable.FancyButton_fb_textStyle, STYLE_NORMAL);

        mIconPosition = a.getInt(R.styleable.FancyButton_fb_iconPosition, mIconPosition);

        textStyle = a.getInt(R.styleable.FancyButton_android_textStyle, Typeface.NORMAL);

        String fontIcon = a.getString(R.styleable.FancyButton_fb_fontIconResource);

        String iconFontFamily = a.getString(R.styleable.FancyButton_fb_iconFont);
        String textFontFamily = a.getString(R.styleable.FancyButton_fb_textFont);

        try {
            mIconResource = a.getDrawable(R.styleable.FancyButton_fb_iconResource);
        } catch (Exception e) {
            mIconResource = null;
        }
        isZoomIcon = a.getBoolean(R.styleable.FancyButton_fb_isZoomIcon, false);
        zoomIconWidth = (int) a.getDimension(R.styleable.FancyButton_fb_iconResource_width, 0);
        zoomIconHeight = (int) a.getDimension(R.styleable.FancyButton_fb_iconResource_height, 0);
        ...
        if (!isInEditMode()) {
            ...
            Typeface fontResource = getTypeface(a);
    ...
    private Typeface getTypeface(TypedArray ta) {
        if (ta.hasValue(R.styleable.FancyButton_android_fontFamily)) {
            int fontId = ta.getResourceId(R.styleable.FancyButton_android_fontFamily, 0);
            if (fontId != 0) {
                try {
        ...
        if (ta.hasValue(R.styleable.FancyButton_fb_textFontRes)) {
            int fontId = ta.getResourceId(R.styleable.FancyButton_fb_textFontRes, 0);
            if (fontId != 0)
                try {
                    return ResourcesCompat.getFont(getContext(), fontId);
                } catch (Exception exception) {
    ...
    private Drawable getRippleDrawable(Drawable defaultDrawable, Drawable focusDrawable, Drawable disabledDrawable) {
        if (!mEnabled) {
            ...
            return new RippleDrawable(ColorStateList.valueOf(isGradient ? mGradientFocusBackgroundStartColor : mFocusBackgroundColor), defaultDrawable, focusDrawable);
        }
    }
    ...
    private void setupBackground() {
        // Default Drawable
        GradientDrawable defaultDrawable = new GradientDrawable(GradientDrawable.Orientation.LEFT_RIGHT, null);
        applyRadius(defaultDrawable);
        if (isGradient) {
            defaultDrawable.setColors(new int[]{
                    mGradientDefaultBackgroundStartColor,
                    mGradientDefaultBackgroundEndColor
            });
        } else {
            if (mGhost)
        ...
        //Focus Drawable
        GradientDrawable focusDrawable = new GradientDrawable(GradientDrawable.Orientation.LEFT_RIGHT, null);
        applyRadius(focusDrawable);

        if (isGradient)
            focusDrawable.setColors(new int[]{
                    mGradientFocusBackgroundStartColor,
                    mGradientFocusBackgroundEndColor
            });
        else
            focusDrawable.setColor(mFocusBackgroundColor);

        // Disabled Drawable
        GradientDrawable disabledDrawable = new GradientDrawable(GradientDrawable.Orientation.LEFT_RIGHT, null);
    ...
    public void setText(@StringRes int res) {
        this.setText(getResources().getString(res));
    }
    ...
    public void setTextSize(int textSize) {
        this.mDefaultTextSize = (int) UtilsKt.spToPx(textSize, getContext());
        if (mTextView != null)
    ...
    public void setFontIconSize(int iconSize) {
        this.mFontIconSize = (int) UtilsKt.spToPx(iconSize, getContext());
        if (mFontIconView != null)
    ...
    private static final Map<String, Typeface> cachedFontMap = new HashMap<String, Typeface>();
